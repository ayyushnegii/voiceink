...
class AudioManager {
  ...
  async processAudio(audioBlob) {
    try {
      // Get window context for tone adjustment
      let context = null;
      try {
        const contextResult = await window.electronAPI.getWindowContext();
        if (contextResult.success) {
          context = contextResult;
          console.log(`🎧 [AudioManager] Window context: ${context.context} (${context.tone})`);
        }
      } catch (e) {
        console.warn('Failed to get window context:', e);
      }
      ...
      const hinglishMode =
        localStorage.getItem("hinglishMode") === "true";
      
      let result;
      if (useLocalWhisper) {
        result = await this.processWithLocalWhisper(audioBlob, whisperModel, context, hinglishMode);
      } else {
        result = await this.processWithOpenAIAPI(audioBlob, context, hinglishMode);
      }
  ...
  async processWithLocalWhisper(audioBlob, model = "base", context = null, hinglishMode = false) {
    ...
    try {
      ...
      if (result.success && result.text) {
        ...
        if (text) {
          ...
          if (useReasoning) {
            try {
              text = await this.processWithReasoningModel(text, context, hinglishMode);
              return { success: true, text, source: "local-reasoned" };
            } catch (reasoningError) {
              console.warn('Reasoning failed, using raw transcription:', reasoningError);
  ...
  async processWithOpenAIAPI(audioBlob, context = null, hinglishMode = false) {
    try {
      ...
      if (rawText) {
        ...
        if (useReasoning) {
          const reasonedText = await this.processWithReasoningModel(rawText, context, hinglishMode);
          return {
            success: true,
            text: reasonedText,
