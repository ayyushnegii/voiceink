...
class IPCHandlers {
  constructor(managers) {
    ...
    this.windowContextDetector = managers.windowContextDetector;
    this.setupHandlers();
  }
  ...
  setupHandlers() {
    ...
    // Window context handler for context-aware tone adjustment
    ipcMain.handle("get-window-context", async (event) => {
      try {
        const contextInfo = this.windowContextDetector.getCurrentContext();
        return {
          success: true,
          context: contextInfo.context,
          windowTitle: contextInfo.windowTitle,
          tone: contextInfo.tone,
          prompt: this.windowContextDetector.getContextPrompt(contextInfo.windowTitle)
        };
      } catch (error) {
